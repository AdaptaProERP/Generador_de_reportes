...
PROCE MAIN(lBegin,oRun,oCursor)
   ...
   LOCAL nSalida
// LOCAL aSalida:={"Previsualización","Impresora","Ventana","Txt Windows","Excel","DBF","Html","Crystal Preview","Crystal Print","Pdf File","Pdf View"}
...
// ? lBegin,"lBegin",oRun:nOut,"oRun:nOut"
   ...
   IF !lBegin
     ...
     //  Salida hacia PDF
     IF !EJECUTAR("REPOUTPUTEND",oRun,oCursor)
       RETURN .T.
     ENDIF

   ENDIF

   oRun:oGenRep:lCrystalP:=(oRun:nOut=10)
   oRun:oGenRep:lCrystalW:=(oRun:nOut=11)
...
// ? oRun:nOut,"OUT",lBegin

//   IF !lBegin .AND. (oRun:nOut=12) // Salida por Browse
//      EJECUTAR("REPTOARRAY",oRun:cFileTxt)
//      RETURN NIL
//   ENDIF

   IF (oRun:nOut=10 .OR. oRun:nOut=11) .AND. lBegin
...
//     ? oRun:nOut,"oRun:nOut, Salida para preview"
...
//     oDp:lMysqlNativo:=.F.
   ...
   IF lBegin .AND. (oRun:nOut=3 .OR. oRun:nOut=4 .OR. oRun:nOut=12) // Salida por TXT
      oRun:cFileTxt:=GETTMPNAME(".TXT")
      oRun:cFileTxt:="TEMP\REP"+LSTR(SECONDS())+".TXT"
   ...
   IF TYPE("oReport")="O" .AND. "PRINT"$oReport:oDevice:ClassName() .AND. !lBegin .AND. (oRun:nOut=10 .OR. oRun:nOut=11)
   ...
   IF lBegin .AND. oRun:nOut=6
      ...
      IF !ValType(oCursor)="O" .AND. !FILE(cFileDbf)
